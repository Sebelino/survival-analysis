...
unscaled residuals. Grambsch and Thereau [@grambsch1994] showed
that scaled residuals have better properties such as
approximately constant variance over time.
This means standard statistical methods can be applied,
including a chi-square test.
We can also consider whether we want to scale time in the residual regression.
Using an identity transform (`transform = "identity"`) regresses scaled Schoenfeld residuals on raw time, which is appropriate when we are interested in assessing or interpreting time-varying effects on the original time scale. Alternatively, the Kaplan-Meier transform (`transform = "km"`) uses the Kaplan-Meier estimate of survival as the time scale, which can improve power and robustness in the presence of heavy or nonuniform censoring. This transformation reflects the amount of information available at each time point and is particularly useful when the hazard function or censoring distribution is highly variable, which may be the case here.
...
to see a non-horizontal pattern in the residuals.
The advantage of this method is that
it is specifically designed for Cox models,
it complements the formal test (using `cox.zph(...)`),
and it can be used for each covariate separately, helping isolate potential violations.
...
colorectal cancer patients. Our exposure variable is binary (aspirin vs.
no aspirin), whereas our response variable is time to cause-specific
death. We are therefore interested in estimating a measure of
association between these two variables. There are several candidates:
hazard ratio, hazard difference, survival probability difference,
restricted mean survival time (RMST) difference. The standard choice of
measure for this type of study would be hazard ratio. Hazard difference
carries the disadvantage of being unstable over time and thus difficult
to interpret, as hazards generally change over time. Survival
probability does not account for competing risks, such as death due to
other causes. RMST difference could be a valuable alternative if hazards
are non-proportional, but requires carefully selecting a suitable time
...
means that we should treat death due to other causes as a competing
risks and censor such events.
...
average, both measured and unmeasured sources of confounding is expected
to be equally distributed across treatment groups, and confounding is
expected to be minimal.

However, we need to consider that the number of subject with both
...
Hence, some confounders may still be unevenly distributed by chance. To
assess confounding, we would check baseline characteristics (age, sex,
stage at diagnosis, etc.) for imbalances between the experimental arm
and the control arm.

We would then produce a baseline table (Table 1) comparing these
characteristics by randomization arm. If strong imbalances exist, we
would stratify on key variables rather than adjusting them as
covariates. We would then switch to a stratified Cox model. The primary
reason for adjusting is to account for meaningful differences that might
threaten the randomization assumption (especially in smaller trials).

We would plot a Kaplan-Meier curve to visualize survival over time for
the two groups. We would then investigate whether the proportional
hazards assumption holds. To do this, we would look at the survival
...
alternative models. We would switch to stratified Cox regression, use
time-dependent Cox models, or consider RMST analysis as an alternative.
